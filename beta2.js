/* FÁBRICA CHAÑAR — experiencia simplificada v6 */
(function(){
  ...
  function simplifyControls(){
    ...
    const nodes=[...controls.children].filter(el=>{
      if(el===studio||el===advanced||el.classList.contains('collection-studio'))return false;
      if(el.id==='status'||el.classList.contains('beta-series-details'))return false;
      return true;
    });
  ...
  function boot(){
    ...
    setTimeout(()=>{
      ...
      setTimeout(simplifyControls,100);
    },120);
  }
