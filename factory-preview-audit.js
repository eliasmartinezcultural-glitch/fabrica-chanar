/* FÁBRICA CHAÑAR — AUDITORÍA PROFUNDA v3
   Verifica producto, catálogo cerrado, realización editorial, materia prima,
   fotografía, capas, identidad, derechos y geometría básica.
   No inventa ni amplía catálogo.
*/
(function(){
  ...
  function catalogAudit(){
    ...
    const total=out.reduce((n,x)=>n+x.count,0),editorialTotal=out.reduce((n,x)=>n+x.editorialCount,0);return {version:3,products:out,totalMatrices:total,totalEditorialRealizations:editorialTotal,pass:out.length===4&&total===40&&editorialTotal===40&&out.every(x=>x.pass),rule:'4 productos × 10 fotos = 40 matrices editoriales realizadas'};
  }
  function run(){
    ...
    add('product-template',box?.classList.contains('master-product-'+product),'Contrato visual de producto aplicado.'); add('front',!!box?.querySelector('.master-front'),'Frente montado.'); add('reverse',!!box?.querySelector('.master-reverse'),'Reverso montado.');
    ...
    add('photo-rights',!!meta.photoSelection?.rights,'Derechos registrados.'); add('photo-selection',!!meta.photoSelection?.asset&&meta.materialPhotoApplied===true,'Materia prima fotográfica aplicada.'); add('identity',!!meta.pieceCode&&!!meta.collectionName&&!!meta.makerSeal,'Identidad Ocarina completa.'); add('photo-render',meta.photoRender?.stage===true&&meta.photoRender?.layerOrder==='art > photo-stage > content > seal','Diagnóstico de capas correcto.'); add('no-placeholder',!box?.querySelector('.photo-missing'),'Sin marcador de foto faltante.');
    add('no-abstract-over-photo',!!img||!box?.querySelector('.original-art-svg'),'Arte abstracto no compite con la foto.');
    const title=rect('.master-skin-title'),fact=rect('.master-fact-plaque'),seal=rect('.master-skin-seal'),credit=rect('.photo-credit-chip'); add('title-fact-overlap',overlap(title,fact)<.18,`solapamiento ${Math.round(overlap(title,fact)*100)}%`); add('fact-seal-overlap',overlap(fact,seal)<.18,`solapamiento ${Math.round(overlap(fact,seal)*100)}%`); add('credit-seal-overlap',overlap(credit,seal)<.12,`solapamiento ${Math.round(overlap(credit,seal)*100)}%`);
    const critical=['catalog-closed','canvas','product','complete-class','product-template','front','reverse','editorial-slot','editorial-content','photo-stage','photo-node','photo-loaded','title','fact','collection','seal','origin','photo-rights','photo-selection','identity','photo-render','no-placeholder'];
    const passed=checks.filter(x=>x.ok).length,criticalPass=critical.every(id=>checks.find(x=>x.id===id)?.ok),result={version:3,product,passed,total:checks.length,score:Math.round(passed/checks.length*100),criticalPass,ready:criticalPass&&checks.filter(x=>!x.ok).length<=3,checks,catalog:ca,masterId:meta.rawMaterial?.masterId||null,photoId:meta.photoId||null,commercialRights:meta.commercialRights===true};
  ...
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot);else boot(); window.FabricaPreviewAudit={version:3,run,catalogAudit,products:PRODUCTS};
})();
