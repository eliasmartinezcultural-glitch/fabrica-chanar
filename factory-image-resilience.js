/* FÁBRICA CHAÑAR — RESILIENCIA VISUAL v2
   La interfaz nunca queda en blanco porque una foto externa no responde.
   Lee el mismo estado global léxico que el runtime principal.
*/
(function(){
  ...
  function styleOf(){const s=getState();const raw=s?.factoryMeta?.style;return raw?.accent?({cielo:'patagonia',uva:'vino',tierra:'cava',cosecha:'vendimia',fruta:'fiesta',coral:'arte'}[raw.accent]||'patagonia'):s?.factoryMeta?.style?.mood||s?.style||'patagonia'}
  function repairImages(root){
    ...
    root.querySelectorAll('img').forEach(img=>{
      ...
      img.addEventListener('error',()=>{
        ...
        const s=getState();
        const label=s?.data?.title||s?.factoryMeta?.masterProduct?.name||'San Patricio del Chañar';
  ...
  function ensurePreview(root){
    ...
    const s=getState();
    root.style.setProperty('--factory-fallback-image','url("'+svgFallback(styleOf(),s?.data?.title||'San Patricio del Chañar')+'")');
  }
  function hook(){
  ...
  window.FabricaImageResilience={version:2,repairImages,ensurePreview};
})();
