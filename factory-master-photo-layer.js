/* FÁBRICA CHAÑAR — FOTO MAESTRA v6
   UNA sola responsabilidad: colocar la fotografía en su escenario.
   Solo actúa cuando existe un producto maestro activo.
*/
(function(){
  ...
  function apply(){
    ...
    const skin=box.querySelector('.master-product-skin'),front=skin?.querySelector('.master-front'); if(!skin||!front)return;
    front.querySelector('.master-photo-stage')?.remove();
    const selected=asset(selectedAssetId()),fallback=asset(mp.photo||m.photo),a=selected||fallback;
    const src=typeof state!=='undefined'&&state.image?state.image:(a?.photo||null);
    const stage=document.createElement('div');stage.className='master-photo-stage';stage.setAttribute('aria-label','Fotografía de la pieza');
    stage.innerHTML=src?`<img src="${esc(src)}" alt="${esc(m.name)}" loading="eager" decoding="async" referrerpolicy="no-referrer">`:`<div class="photo-missing"><b>FOTO NO DISPONIBLE</b><span>La pieza conserva su estructura hasta incorporar una imagen válida.</span></div>`;
    front.prepend(stage);box.classList.add('master-photo-directed');box.classList.toggle('master-photo-missing',!src);box.querySelectorAll('.original-art-svg').forEach(el=>{el.hidden=!!src});
    ...
    state.factoryMeta={...(state.factoryMeta||{}),photoDirection:{version:6,id:selectedAssetId()||m.photo,focus:m.photoFocus||meta().photoSelection?.focus,crop:m.photoCrop||meta().photoSelection?.crop,position:meta().photoSelection?.position||m.photoPosition,role:m.photoRole||meta().photoSelection?.role,source:m.photoSource||a?.source,rights:m.rights||meta().photoSelection?.rights,license:m.photoLicense||a?.license||'',author:m.photoAuthor||a?.author||''},photoRender:{version:6,stage:true,layerOrder:'photo-stage > content',src:!!src,asset:selectedAssetId()||m.photo,loaded:!!img?.complete&&!!img?.naturalWidth},commercialRights:['own','usable','licensed','authorized'].includes(m.rights||meta().photoSelection?.rights)};
  }
  ...
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot);else boot();window.FabricaMasterPhotoLayer={version:6,apply,hook};
})();