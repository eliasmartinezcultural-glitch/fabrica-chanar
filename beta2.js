/* FÁBRICA CHAÑAR — experiencia simplificada v8
   La salida comercial se habilita por derecho explícito, no por la mera presencia de una imagen.
*/
(function(){
  ...
  function injectInterfaceCSS(){
    ...
    const style=document.createElement('style');style.id='fabrica-simplified-ui';style.textContent=`
      ...
      .beta-mode .hero{padding-top:22px;padding-bottom:28px}.beta-mode .hero h1{text-shadow:none}.beta-mode .hero-rule span,.beta-mode .hero-rule strong{box-shadow:none}
      ...
      .beta-series-details>.collection-studio{margin:0;border:0;border-radius:0;box-shadow:none;background:transparent;padding:14px}.beta-advanced-details{margin-top:12px}.beta-advanced-details>.step:first-child{margin-top:14px}.beta-advanced-details>.actions{margin:14px}.beta-advanced-details>.status{margin:0 14px 12px}.factory-studio{margin-bottom:10px}
  ...
  function simplifyControls(){
    ...
    nodes.forEach(el=>advanced.appendChild(el));controls.appendChild(advanced);if(statusEl)controls.insertBefore(statusEl,advanced);controls.dataset.simplified='1';
  }
  function guardDownload(e){
    if(!commercialCurrent()){
      e.preventDefault();e.stopImmediatePropagation();
      status('Para descargar esta pieza para venta, usá una foto propia/autorizada/licenciada o un arte comercial de la Fábrica.');
      setFactoryState('working','La Fábrica protegió la salida: esta pieza todavía no tiene un derecho comercial explícito.');
      return false;
    }
  ...
  function boot(){
    ...
    ['btnPng','btnJpg','btnPrint'].forEach(id=>{const b=document.getElementById(id);if(!b)return;b.addEventListener('click',guardDownload,true)});
    setTimeout(simplifyControls,260);
    setTimeout(()=>{if(typeof renderProducts==='function')renderProducts();if(typeof renderTemplates==='function')renderTemplates();if(typeof renderForm==='function')renderForm();if(typeof renderPreview==='function')renderPreview();window.FabricaMasterVisuals?.apply?.();setTimeout(simplifyControls,100)},120);
  }
