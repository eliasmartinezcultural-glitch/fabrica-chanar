/* FÁBRICA CHAÑAR — LENGUAJE EDITORIAL Y SELLOS v7
   Texto universal bloqueado por producto. La fotografía es intercambiable.
   Toda marca, textura y contenido visual debe vivir dentro de .piece.
*/
(function(){
  ...
  function getState(){return window.state||window.factoryState||null}
  function product(){const s=getState();return s?.type||s?.centralProduct||document.querySelector('#canvasPreview .piece')?.dataset?.product||'postal'}
  ...
  function installArtCSS(){
    ...
    const s=document.createElement('style');s.id='factory-art-seal-system';s.textContent=`
      #canvasPreview{overflow:hidden!important;display:flex!important;justify-content:center!important;align-items:flex-start!important}
      #canvasPreview .piece{position:relative!important;overflow:hidden!important;isolation:isolate!important;clip-path:inset(0)!important}
      #canvasPreview .piece>*{max-width:100%;min-width:0}
      ...
      .piece-image{overflow:hidden!important;min-width:0!important}.piece-image img{width:100%!important;height:100%!important;object-fit:cover!important;object-position:center center!important;display:block!important;image-rendering:auto!important}
      .piece.postal .piece-image img{object-position:50% 45%!important}.piece.ficha .piece-image img{object-position:50% 40%!important}.piece.guide .piece-image img{object-position:50% 42%!important}.piece.infographic .piece-image img{object-position:50% 38%!important}
  ...
  function addSeal(piece,p){piece.querySelectorAll('.factory-art-seal').forEach(x=>x.remove());const seal=document.createElement('div');seal.className='factory-art-seal factory-art-seal-'+p;seal.innerHTML='<span class="factory-art-seal-star">✦</span><b>CHAÑAR</b><small>PIEZA CURADA</small><em>OCARINA</em>';piece.appendChild(seal)}
  function normalize(){
    const box=document.querySelector('#canvasPreview'),piece=box?.querySelector('.piece');if(!box||!piece)return;
    ...
    const s=getState();if(s)s.factoryMeta={...(s.factoryMeta||{}),pieceLanguageVersion:7,editorialMode:'universal-locked',editorialRule:'Texto fijo por producto; la fotografía es intercambiable y no modifica el lenguaje editorial.',textContractLocked:true,artDirectionVersion:7,sealSystem:'chañar-curated',physicalContainment:'all-art-inside-piece'};
    addSeal(piece,p)
  }
  ...
  window.FabricaPiece={version:7,locked:true,phrases:UNIVERSAL,refresh:apply,universal:UNIVERSAL,artDirection:7};
})();
