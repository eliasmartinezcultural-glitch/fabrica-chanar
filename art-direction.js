/* FÁBRICA CHAÑAR · DIRECCIÓN ARTÍSTICA V1.1
   Capa visual sobre el único motor existente.
   No fabrica ni duplica runtime: mejora presentación, tarjeta, reverso y selección.
*/
(()=>{'use strict';
const PALETTES={
...
/* ART DIRECTION LOCK — producto local con lenguaje editorial/artesanal */
.art-tools{display:flex;justify-content:center;align-items:center;gap:7px;flex-wrap:wrap;margin:0 0 8px}
.art-tools .btn{font-size:10px;min-width:74px}.art-tools .btn.active{background:#171713;color:#fff;border-color:#171713}
.art-tools .status{margin:0;padding:5px 8px;border:1px solid #d8cdbc;border-radius:99px;background:#faf6ed}
...
.art-badges{position:absolute;left:18px;top:18px;display:flex;gap:5px;flex-wrap:wrap;max-width:72%;z-index:9;pointer-events:none}
.art-badge{padding:6px 8px;border:1px solid var(--art-gold,#b58a4f);background:color-mix(in srgb,var(--art-paper,#fff) 90%,transparent);color:var(--art-ink,#2d2822);font:900 7px/1 system-ui,sans-serif;letter-spacing:.12em;text-transform:uppercase;box-shadow:0 3px 8px #0002}
...
.back-title{font:700 clamp(24px,4vw,38px)/1 Georgia,serif;margin:10px 0 5px;max-width:88%;overflow-wrap:anywhere}
.back-sub{font:800 11px/1.35 system-ui,sans-serif;color:color-mix(in srgb,var(--art-ink,#2d2822) 78%,transparent);max-width:85%;overflow-wrap:anywhere}
...
.back-copy{font:11px/1.45 system-ui,sans-serif;max-width:88%;color:color-mix(in srgb,var(--art-ink,#2d2822) 84%,transparent);overflow-wrap:anywhere}
...
/* Las tarjetas de catálogo también hablan el mismo lenguaje que la pieza fabricada. */
.curated-card{padding:12px 11px 10px;min-height:132px;border-radius:12px;background:linear-gradient(145deg,#fffdf8,var(--bg));}
.curated-card:after{content:'MUY LOCAL · ARTESANAL';position:absolute;right:9px;bottom:8px;font:900 6px/1 system-ui,sans-serif;letter-spacing:.12em;color:var(--c);opacity:.72}
.curated-card.chosen{outline:1px solid color-mix(in srgb,var(--c) 40%,transparent);outline-offset:2px}
.curated-card .curated-family{padding-right:66px}.curated-card .curated-seal{font-size:6px;padding:5px 7px}
.art-card-meta{display:flex;gap:4px;flex-wrap:wrap;margin-top:6px;padding-right:72px}
.art-mini{font:900 6px/1 system-ui,sans-serif;letter-spacing:.08em;text-transform:uppercase;padding:4px 5px;border:1px solid color-mix(in srgb,var(--c) 45%,#b9aa94);border-radius:99px;color:var(--c);background:#fffaf0}
.photo-bank .photo{transition:transform .12s ease,outline .12s ease}.photo-bank .photo:not(.pending):hover{transform:translateY(-1px)}
@media(max-width:520px){.art-back{padding:27px}.back-grid{grid-template-columns:1fr}.back-title{font-size:25px}.art-badges{left:15px;top:15px;max-width:78%}.art-badge{font-size:6px;padding:5px 6px}.curated-card:after{font-size:5px}}
`;document.head.appendChild(style);
function $(id){return document.getElementById(id)}
function selectedPhoto(){return document.querySelector('#photos .photo.active')||null}
...
function enhanceCurated(){document.querySelectorAll('.curated-card').forEach(card=>{if(card.querySelector('.art-card-meta'))return;const m=document.createElement('div');m.className='art-card-meta';m.innerHTML='<span class="art-mini">Local</span><span class="art-mini">Artesanal</span><span class="art-mini">Turismo</span>';card.appendChild(m)})}
...
window.FABRICA_ART_DIRECTION={version:'1.1',rule:'arte local + turismo + reverso vivo + tarjetas coherentes'};
})();