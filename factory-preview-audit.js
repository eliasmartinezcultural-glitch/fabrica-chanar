/* FÁBRICA CHAÑAR — AUDITORÍA PROFUNDA v4
...
(function(){
  ...
  function catalogAudit(){
    ...
    const total=out.reduce((n,x)=>n+x.count,0),editorialTotal=out.reduce((n,x)=>n+x.editorialCount,0);return {version:4,products:out,totalMatrices:total,totalEditorialRealizations:editorialTotal,pass:out.length===4&&total===40&&editorialTotal===40&&out.every(x=>x.pass),rule:'4 productos × 10 fotos = 40 matrices editoriales realizadas'};
  }
  function run(){
    ...
    add('photo-rights',!!meta.photoSelection?.rights||!!meta.provenance?.rights||!!meta.provenance?.photoRights,'Derechos/procedencia registrados.'); add('photo-selection',!!meta.photoSelection?.asset&&meta.materialPhotoApplied===true,'Materia prima fotográfica aplicada.'); add('identity',!!meta.pieceCode&&!!meta.collectionName&&!!meta.makerSeal,'Identidad Ocarina completa.');
  ...
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot);else boot(); window.FabricaPreviewAudit={version:4,run,catalogAudit,products:PRODUCTS};
})();