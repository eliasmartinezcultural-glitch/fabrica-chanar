/* FÁBRICA CHAÑAR — DIRECCIÓN DE PRODUCTO CERRADA v7
   UNA COLECCIÓN / CUATRO FORMATOS / UNA SUPERFICIE
   El sistema maestro solo actúa cuando existe un producto maestro activo.
   Nunca borra ni reemplaza una pieza del flujo central.
*/
(function(){
  const RECIPES={
  ...
  function apply(){const box=$('#canvasPreview'),m=master();if(!m)return;const r=recipe(m);if(!box||!r)return;removeLegacy(box);cleanClasses(box);const shell=ensure(box),l=law(m);shell.innerHTML=`<div class="master-front">${front(m,l)}<div class="master-photo-stage" data-role="master-photo-stage"></div></div><div class="master-reverse" aria-hidden="true">${reverse(m,l)}</div>`;box.classList.add('master-product-preview','master-product-'+r.product,'master-'+r.family);box.dataset.masterId=m.id;box.dataset.masterFormat=r.format;let toggle=$('#btnMasterFlip');if(!toggle){toggle=document.createElement('button');toggle.id='btnMasterFlip';toggle.type='button';toggle.className='master-flip';const host=box.closest('.preview-panel')?.querySelector('.preview-head');host?.appendChild(toggle)}toggle.textContent='↻ Ver reverso';toggle.onclick=()=>{const rev=box.classList.toggle('master-show-reverse');toggle.textContent=rev?'↻ Ver frente':'↻ Ver reverso'};if(typeof state!=='undefined')state.factoryMeta={...(state.factoryMeta||{}),masterVisual:{version:7,locked:true,isolated:true,recipe:r,format:r.format,reverse:true,identity:'clean',decorations:false}}}
  function boot(){['fabrica:ready','fabrica:series-ready','fabrica:masters-ready'].forEach(ev=>document.addEventListener(ev,()=>setTimeout(apply,60)));document.getElementById('btnGenerate')?.addEventListener('click',()=>setTimeout(apply,160));setTimeout(apply,900)}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot);else boot();window.FabricaMasterVisuals={version:7,locked:true,isolated:true,apply,clear,recipes:RECIPES,products:['postal','ficha','guide','infographic']};
})();