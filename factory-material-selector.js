/* FÁBRICA CHAÑAR — SELECTOR CERRADO v4
...
(function(){
  ...
  function select(product){
    ...
    return {product,slot:slot?.slot||1,closedOptions:closed.length,material,master,score:material?(window.FabricaRawMaterials?.materialScore?.(material,product)||0):0,fit:material?fitFor(material,product):0,sourceType:'closed-catalog',photoAsset:slot?.photo||null};
  }
  ...
  function attach(selection,product){
    ...
    const material=selection.material,master=selection.master,asset=assetForId(selection.photoAsset)||assetForId(material?.photo?.asset);
    ...
    state.factoryMeta={...(state.factoryMeta||{}),
      rawMaterial:{version:4,masterId:material.master||'',theme:material.theme||'',score:selection.score,fit:selection.fit,sourceType:selection.sourceType,photo:material.photo||null,facts:material.facts||[],microstory:material.microstory||'',sources:material.sources||[],credit:material.credit||''},
      ...id,
      ...
      photoSelection:{asset:asset?.id||selection.photoAsset||null,role:material.photo?.role||'editorial',focus:catalog()?.get?.(product,selection.slot)?.direction||material.photo?.focus||'',crop:material.photo?.crop||'',rights:asset?.kind||material.photo?.rights||'unknown',commercialSafe:asset?.kind==='usable'||asset?.kind==='own'||asset?.kind==='authorized'||asset?.kind==='licensed'||material.photo?.commercialSafe===true},
      selectedFact:fact,
    ...
    if(asset?.photo){
      ...
      state.factoryMeta.photoSelection.source=asset.source||'';
      state.factoryMeta.photoSelection.credit=asset.author||material.credit||'';
    }
    if(closedOptionsReady(product)){
      state.factoryMeta.closedCatalogComplete=true;
    }
  }
  ...
  function install(){
    ...
    window.FabricaEngine.produce=async function(options={}){
      ...
      return {...result,rawMaterial:selected.material,selectedMaster:selected.master,materialScore:selected.score,materialFit:selected.fit,materialSourceType:selected.sourceType,closedCatalogSlot:selected.slot,closedCatalogOptions:selected.closedOptions,closedCatalogPhoto:selected.photoAsset};
    };
    window.FabricaEngine.selectMaterial=select;
    window.FabricaEngine.materials=materials;
    window.FabricaEngine.materialSelectorVersion=4;
  }
  function selfTest(){
    const results=PRODUCTS.map(product=>{const c=catalog()?.forProduct?.(product)||[];const s=select(product);return {product,template:catalog()?.products?.[product]?.template||product,options:c.length,master:s.master?.id||null,photo:s.photoAsset,pass:c.length===10&&!!s.material&&!!s.photoAsset}});
    return {version:4,ok:results.every(x=>x.pass),results,summary:catalog()?.summary?.()||null,rule:'4 plantillas × 10 fotos = 40 matrices cerradas'};
  }
  document.addEventListener('DOMContentLoaded',()=>setTimeout(install,700));
  window.FabricaMaterialSelector={version:4,select,masterFor,attach,install,selfTest};
})();
