/* FÁBRICA CHAÑAR — CAPA FOTOGRÁFICA MAESTRA v4
   Contrato: fondo conceptual → escenario fotográfico → contenido → sello.
   La foto nunca se inserta directamente en .master-front: tiene escenario propio.
*/
(function(){
  ...
  function apply(){
    ...
    const skin=box.querySelector('.master-product-skin'); const front=skin?.querySelector('.master-front');
    if(!skin||!front)return;
    skin.querySelector('.master-photo-stage')?.remove();
    ...
    stage.setAttribute('aria-label','Escenario fotográfico');
    stage.innerHTML=src
    ...
    if(img){
      ...
      img.addEventListener('error',()=>{box.classList.remove('master-photo-loaded');box.classList.add('master-photo-missing');stage.classList.add('photo-load-error');img.remove();stage.insertAdjacentHTML('afterbegin','<div class="photo-missing"><b>FOTO NO DISPONIBLE</b><span>Conservar referencia y solicitar original/autorización.</span></div>');},{once:true});
    }
    state.factoryMeta={...(state.factoryMeta||{}),photoDirection:{version:4,id:selectedAssetId()||m.photo,focus:m.photoFocus||meta().photoSelection?.focus,crop:m.photoCrop||meta().photoSelection?.crop,position:meta().photoSelection?.position||m.photoPosition,role:m.photoRole||meta().photoSelection?.role,source:m.photoSource||a?.source,rights:m.rights||meta().photoSelection?.rights,license:m.photoLicense||a?.license||'',author:m.photoAuthor||a?.author||''},photoRender:{version:4,stage:true,layerOrder:'art > photo-stage > content > seal',src:!!src,asset:selectedAssetId()||m.photo,loaded:!!img?.complete&&!!img?.naturalWidth},commercialRights:['own','usable','licensed','authorized'].includes(m.rights||meta().photoSelection?.rights)};
  }
  ...
  window.FabricaMasterPhotoLayer={version:4,apply,hook};
})();
